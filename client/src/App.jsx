...
// --- FILTERING & STRICT SORTING LOGIC ---
  const filteredLogs = useMemo(() => {
    ...
    // 2. Apply Strict Sorting Rules
    return filtered.sort((a, b) => {
      // RULE 1: STATUS PRIORITY (High to Low)
      // Watching (3) > Completed (2) > Dropped (1)
      const priority = { WATCHING: 3, COMPLETED: 2, DROPPED: 1 };
      const weightA = priority[a.status] || 2; // Default to Completed if missing
      ...
      // RULE 2: WATCHING SPECIFIC (Progress % Descending)
      if (a.status === 'WATCHING') {
        ...
        // If progress is different, highest % goes first
        if (Math.abs(progA - progB) > 0.01) {
          return progB - progA;
        }
      }

      // RULE 3: DATE FALLBACK (Most Recent First)
      // Applies to Completed, Dropped, and Watching items with equal progress
      const dateA = new Date(a.date_watched).getTime();
      const dateB = new Date(b.date_watched).getTime();
      return dateB - dateA;
    });
  ...
  const availableYears = [
    ...
    // Extract the first 4 digits to sort by start year
    const yearA = parseInt(String(a).substring(0, 4));
    const yearB = parseInt(String(b).substring(0, 4));
    return yearB - yearA;
  });
  ...
  const handleSubmit = async (e) => {
    ...
    // RULE 1: DROPPED always wins. We save exactly what the user typed.
    if (finalStatus !== "DROPPED" && total > 0) {
      // RULE 2: Numbers don't lie. If you type the max, it is Completed.
      if (ep === total) {
        finalStatus = "COMPLETED";
      }

      // RULE 3: Handling Partial Progress (ep < total)
      else if (ep < total) {
        // Logic: Did the user select "Completed"?
        if (finalStatus === "COMPLETED") {
          // We need to distinguish between "I want to finish this" vs "I am correcting a number".
          // We check the previous state (originalLog) to see if the Status was just changed.
          const originalLog = logs.find((l) => l.id === editingId);
          const isStatusChange = originalLog
            ? originalLog.status !== "COMPLETED"
            : true;

          // CONDITION A: Lazy Complete / Status Change
          // If the episode is 0, OR this is a new entry, OR the user just switched status to COMPLETED:
          // We assume they want to "Complete" the show.
          if (ep === 0 || isStatusChange) {
            ep = total; // Auto-fill to max
          }

          // CONDITION B: Rewatch / Correction
          // If the status was ALREADY Completed, but the user manually typed a lower number:
          // We assume they are correcting progress or rewatching.
          else {
            finalStatus = "WATCHING";
          }
        }

        // Logic: If status is WATCHING, we trust the numbers (do nothing)
      }
    }
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="grid grid-cols-4 gap-4">
                ...
                <div>
                  ...
                  <input
                    type="text"
                    placeholder="e.g. 2011 or 2011-2019"
                    className="w-full bg-stone-900 border border-stone-700 rounded-lg p-2.5 text-white focus:border-stone-500 outline-none"
